...
export function apply(ctx) {
  ...
  // ---------------- 会话工作目录（工具执行上下文） ----------------
  // exec.agent.session.header.cwd 是发起调用的会话工作目录；无 exec 时回退 sandboxPolicy。
  let sessionCwd = null
  function execSessionCwd(exec) {
    try {
      const ag = exec && exec.agent
      if (ag && ag.session && ag.session.header && typeof ag.session.header.cwd === "string") return ag.session.header.cwd
    } catch (e) { /* ignore */ }
    return null
  }
  ...
  // ---------------- SOC2/GDPR 合规自检 ----------------
  function complianceReport(cwdOverride) {
    ...
    const workspaceRoot = (typeof cwdOverride === "string" && cwdOverride) || (sandbox && typeof sandbox.workspaceRoot === "string" ? sandbox.workspaceRoot : null) || null
  ...
  function publishStatus() {
    ...
    try {
      ...
      policy = readPolicy()
      const r = complianceReport(sessionCwd)
  ...
  ctx.on("tools/result", (exec, result) => {
    try {
      ...
      sessionCwd = execSessionCwd(exec) // 记录会话工作目录，供发布/扫描使用
  ...
  ctx.tools.register(defineTool({
    ...
    execute(args, exec) {
      policy = readPolicy()
      const r = complianceReport(execSessionCwd(exec))
      recordHistory(r)
      publishStatus()
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // 优先用发起调用的会话工作目录（exec.agent.session.header.cwd），否则回退 sandboxPolicy。
      const root = execSessionCwd(exec) || workspaceRoot
      const abs = isAbsolute(target) ? target : (root ? resolve(root, target) : resolve(target))
      if (!allowOutside && root) {
        const rr = resolve(root)
        const r = relative(rr, abs)
